...
import redis
#from MQTTClient import pubMQTTMsg
...
def setAlarmValue(pathDB, sql, field, value, mqttTopic =''):
    global ACTIVE
    global INACTIVE
    global NIGHTMODE
    global redisAlarmSetRequest
    global numRaspis
    global payloadAlarmaON
    global payloadAlarmaOFF
    ...
    try:
        if (value == NIGHTMODE):
            ...
            if (value):
                alarmStatus = ACTIVE
                mqttPayload = payloadAlarmaON
            else:
                alarmStatus = INACTIVE
                mqttPayload = payloadAlarmaOFF
        ...
        cur.close()
        DB.close()
        if ((mqttTopic !='') and ((mqttPayload !=''))):
            pubMQTTMsg(mqttTopic, mqttPayload)
